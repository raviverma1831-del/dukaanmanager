...
export default function Purchases({ shop }) {
  ...
  const [form, setForm] = useState({ supplier_id:'', supplier_name:'', purchase_date: new Date().toISOString().slice(0,10), items:[], note:'' })
  ...
  const addItem = (prod) => {
    setForm(f => {
      ...
      return { ...f, items: [...f.items, { product_id: prod.id, product_name: prod.name, qty: 1, price: prod.retail_price, unit: prod.unit }] }
    })
    setSearchProd('')
  }
  ...
  const savePurchase = async () => {
    ...
    const { data: pur, error } = await supabase.from('purchases').insert({
      ...
      total, purchase_date: form.purchase_date,
      status: 'received', note: form.note
    }).select().single()

    if (error) { alert('Error: ' + error.message); setSaving(false); return }

    // Purchase items save + stock update
    await supabase.from('purchase_items').insert(
      form.items.map(i => ({ purchase_id: pur.id, product_id: i.product_id, product_name: i.product_name, quantity: i.qty, price: i.price }))
    )
    ...
    for (const item of form.items) {
      ...
      if (prod) {
        await supabase.from('products').update({ stock: (prod.stock||0) + item.qty }).eq('id', prod.id)
      }
    }

    setForm({ supplier_id:'', supplier_name:'', purchase_date: new Date().toISOString().slice(0,10), items:[], note:'' })
    setShowForm(false); setSaving(false)
    loadAll()
    alert('Purchase saved aur stock update ho gaya!')
  }
  ...
  const filteredProds = products.filter(p => p.name.toLowerCase().includes(searchProd.toLowerCase()) && !p.is_service)
      ...
      {showForm && (
        <div style={{ background:'#fff', borderRadius:18, padding:24, marginBottom:20, boxShadow:'0 4px 20px rgba(0,0,0,0.08)', border:`2px solid ${C.border}` }}>
                  ...
                  {form.items.map(item=>(
                    <tr key={item.product_id} style={{ borderBottom:`1px solid ${C.gXL}` }}>
                      ...
                      <td style={{ padding:'8px 10px' }}>
                        <input type="number" value={item.qty} onChange={e=>updateItem(item.product_id,'qty',+e.target.value)} style={{ ...inp, width:70, textAlign:'center' }}/>
                      </td>
                      <td style={{ padding:'8px 10px', color:C.muted }}>{item.unit}</td>
                      <td style={{ padding:'8px 10px' }}>
                        <input type="number" value={item.price} onChange={e=>updateItem(item.product_id,'price',+e.target.value)} style={{ ...inp, width:90 }}/>
                      </td>
          ...
          <div style={{ display:'flex', gap:10 }}>
            <button onClick={savePurchase} disabled={saving||!form.items.length} style={{ background:saving||!form.items.length?C.muted:`linear-gradient(135deg,#14532d,#16a34a)`, color:'#fff', border:'none', borderRadius:12, padding:'12px 28px', fontFamily:"'Baloo 2',cursive", fontWeight:900, fontSize:15, cursor:saving||!form.items.length?'not-allowed':'pointer' }}>
            ...
            <div key={p.id}>
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'14px 20px', borderBottom:`1px solid ${C.gXL}`, cursor:'pointer' }} onClick={()=>viewId===p.id?setViewId(null):loadPurchaseItems(p.id)}>
                <div>
                  ...
                  <div style={{ fontSize:12, color:C.muted, marginTop:2 }}>{p.purchase_date} {p.note && `• ${p.note}`}</div>
                </div>
