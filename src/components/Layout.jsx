...
const TABS = [
  { id: 'dashboard',   emoji: '🏠', label: 'Dashboard',    group: 'main' },
  { id: 'ai',          emoji: '🤖', label: 'AI Chat',      group: 'main' },
  { id: 'billing',     emoji: '🧾', label: 'Billing',      group: 'accounting' },
  { id: 'vouchers',    emoji: '📒', label: 'Vouchers',     group: 'accounting' },
  { id: 'inventory',   emoji: '📦', label: 'Inventory',    group: 'stock' },
  { id: 'purchases',   emoji: '🛒', label: 'Purchases',    group: 'stock' },
  { id: 'khata',       emoji: '👥', label: 'Khata',        group: 'parties' },
  { id: 'suppliers',   emoji: '🚚', label: 'Suppliers',    group: 'parties' },
  { id: 'fixedassets', emoji: '🏢', label: 'Fixed Assets', group: 'accounts' },
  { id: 'capital',     emoji: '💰', label: 'Capital',      group: 'accounts' },
  ...
  return (
    <div style={{ fontFamily: "'DM Sans',sans-serif", minHeight: '100vh', background: '#f0fdf4' }}>
      ...
      <header style={{ background: `linear-gradient(135deg,#14532d 0%,#16a34a 100%)`, padding: '0 16px', boxShadow: '0 4px 20px rgba(0,0,0,0.15)', position: 'sticky', top: 0, zIndex: 100 }}>
        <div style={{ maxWidth: 1600, margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 56, gap: 8 }}>
          {/* Logo */}
          ...
          {/* Nav */}
          <nav style={{ display: 'flex', gap: 1, flexWrap: 'wrap', alignItems: 'center', flex: 1, justifyContent: 'center', overflowX: 'auto' }}>
            {TABS.map(t => (
              <button key={t.id} onClick={() => setTab(t.id)} style={{
                ...
                fontWeight: tab === t.id ? 800 : 500, fontSize: 10,
